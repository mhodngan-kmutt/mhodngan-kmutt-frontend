...
const VALID_PROFESSOR_EMAIL = process.env.VALID_PROFESSOR_EMAIL!;
const VALID_PROFESSOR_PASSWORD = process.env.VALID_PROFESSOR_PASSWORD!;
const VALID_PROFESSOR_DISPLAY_NAME = process.env.VALID_PROFESSOR_DISPLAY_NAME!;

test('Professor can sign in to certify a project that contributor created, then sign out', async ({ page }) => {
  // ============================================
  // Step 1: Sign in as professor
  // ============================================
  ...
  if (isEmailVisible) {
    await emailInput.fill(VALID_PROFESSOR_EMAIL);
  ...
  await passwordInput.fill(VALID_PROFESSOR_PASSWORD);
  await page.getByRole('button', { name: 'Next' }).click({ timeout: 15000 });
  ...
  await expect(profileMenu).toContainText(VALID_PROFESSOR_DISPLAY_NAME, { timeout: 5000 });
